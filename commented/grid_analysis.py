...
def compute_context_and_complement(g: Grid, O: Cell) -> Tuple[Set[Cell], Set[Cell]]:
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            #contesto (tipo 1) se la distanza orizzontael dx � maggiore o uguale a quella verticale dy
            #complemento (tipo 2) se invece dy � maggiore di dx
            #� un modo semplice per dire se il cammino libero � di tipo 1 o tipo 2
            if dx >= dy:
                context.add((r,c))
            else:
                complement.add((r,c))
