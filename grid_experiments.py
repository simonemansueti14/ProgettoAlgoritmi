...
def compute_context_and_complement_variant(g: Grid, O: Cell, variant:int=0):
    ...
    #scorre tutte le celle della griglia, se la cella non è libera la salta
    for r in range(rows):
        for c in range(cols):
            ...
            if dx==0 and dy==0: #se differenza è 0 allora la cella corrente è O, quindi non deve essere considerata
                continue
...
#trials è quante volte ripetere esperimento
#l'output è un dizionario con i risultati
def experiment(g: Grid, O: Cell, D: Cell, trials:int=1, variant:int=0) -> Dict:
    ...
    #in questo modo quando cammino_minimo chiamerà compute_context_and_complement userà la versione scelta
...
#lancia più esperimenti su una lista di griglie
#grids è la lista di percorsi a file .csv delle griglie
